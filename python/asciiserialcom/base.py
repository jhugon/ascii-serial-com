...
class Base:
    ...
    def check_register_number(self, num: Union[int, str, bytes, bytearray]) -> bytes:
        """
    ...
    def check_register_content(
        self, content: Union[int, str, bytes, bytearray]
    ) -> bytes:
        """
        ...
        """
        registerByteWidth = int(math.ceil(self.registerBitWidth / 8))
    ...
    def convert_to_hex(
        self, num: Union[bytes, bytearray, str, int], N: int = 2
    ) -> bytes:
        """
    ...
    def convert_from_hex(self, num: Union[bytes, bytearray, str, int]) -> int:
        if isinstance(num, int):
            return num
        else:
            if len(num) == 0:
